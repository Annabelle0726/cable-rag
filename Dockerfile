...
# Tika runs as a Java server that the Python client spawns per request. Force UTF-8
# and a non-localized locale so legacy .doc/.ppt text extraction cannot come back
# encoded to the host code page (a GBK container otherwise mangles CJK output).
...
# python-pptx:   default-jre-headless                     tika-server-standard-3.3.0.jar
...
RUN --mount=type=cache,id=ragflow_apt,target=/var/cache/apt,sharing=locked \
    ...
    libglib2.0-0 libglx-mesa0 libgl1 pkg-config libgdiplus default-jre-headless libatk-bridge2.0-0 \
