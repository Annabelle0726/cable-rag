...
from api.db import PIPELINE_SPECIAL_PROGRESS_FREEZE_TASK_TYPES, FileType, CanvasCategory
from api.db.db_models import DB, Document, Knowledgebase, Task, Tenant, File2Document, File, UserCanvas, User
...
class DocumentService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def writable(cls, doc_id, user_id):
        """Whether `user_id` may change one document.

        A document has no authority of its own: it inherits the dataset's, so
        this is the dataset write gate reached through the document.
        """
        e, doc = cls.get_by_id(doc_id)
        if not e:
            return False
        return KnowledgebaseService.writable(doc.kb_id, user_id)
