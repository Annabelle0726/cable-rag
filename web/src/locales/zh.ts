export default {
  translation: {
    ...
    skills: {
      ...
      deleteSelectedDescription: '将删除选中的 {{count}} 个技能空间及其版本。',
    ...
    knowledgeDetails: {
      ...
      rerankTip:
        '非必选项：若不选择 Rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 Rerank 模型，则混合查询中的向量相似度部分将被 Rerank 打分替代。请注意：采用 Rerank 模型会非常耗时。如需选用 Rerank 模型，建议使用 SaaS 的 Rerank 模型服务；如果你倾向使用本地部署的 Rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 RAGFlow。',
      topK: 'Top-K',
      ...
      html4excel: '表格转 HTML',
      html4excelTip:
        '与 General 切片方法配合使用。未开启状态下，表格文件（XLSX、XLS（Excel 97-2003））会按行解析为键值对。开启后，表格文件会被解析为 HTML 表格。若原始表格超过 12 行，系统会自动按每 12 行拆分为多个 HTML 表格。欲了解更多详情，请参阅 https://ragflow.io/docs/dataset_configuration#other-format-processing-configuration。',
      autoKeywords: '自动关键词提取',
      autoKeywordsTip:
        '自动为每个文本块中提取 N 个关键词，用以提升查询精度。请注意：该功能采用在“配置”中指定的索引模型提取关键词，因此也会产生更多 Token 消耗。另外，你也可以手动更新生成的关键词。详情请见 https://ragflow.io/docs/dataset_configuration#content-enhancement-configuration。',
      autoQuestions: '自动问题提取',
      autoQuestionsTip:
        '利用在“配置”中指定的索引模型 对知识库的每个文本块提取 N 个问题以提高其排名得分。请注意，开启后将消耗额外的 Token。您可以在块列表中查看、编辑结果。如果自动问题提取发生错误，不会妨碍整个分块过程，只会将空结果添加到原始文本块。详情请见 https://ragflow.io/docs/dataset_configuration#content-enhancement-configuration。',
      autoTags: '自动标签提取',
      ...
      pleaseSelectFileList: '请选择非空的文件列表',
    },
    knowledgeConfiguration: {
      ...
      imageLoadFailed: '图片加载失败',
      audio:
        '<p>支持的文件格式为 <b>WAV、MP3、AAC、FLAC、OGG</b> 及其他常见音频格式。</p>\n<p>本方法使用语音转文本模型将音频文件转录为文本。</p>',
      email:
        '<p>支持的文件格式为 <b>EML</b> 和 <b>MSG</b>。</p>\n<p>本方法解析电子邮件文件，提取头部字段（如发件人、收件人、抄送、主题和日期）、正文内容以及附件。</p>',
      knowledgeCompiler:
        '<p>本 pipeline 先解析文件并分块，然后通过 Knowledge Compiler 组件将分块编译为结构化知识单元（知识图谱、百科、RAPTOR、思维导图或数据集导航）。</p>\n<p>编译后的知识单元以分块形式合并进分块流输出，适合在分块文档之上构建可检索的知识层。</p>',
      ...
      pageRank: '页面排名',
      pageRankTip:
        '知识库检索时，你可以为特定知识库设置较高的 PageRank 分数，该知识库中匹配文本块的混合相似度得分会自动叠加 PageRank 分数，从而提升排序权重。详见 https://ragflow.io/docs/dataset_configuration#basic-information。',
      tagName: '标签',
      tagMessage: '请选择标签',
      frequency: '频次',
      searchTags: '搜索标签',
      ...
      resolutionTip:
        '解析过程会将具有相同含义的 Entities 合并在一起，从而使 Graph 更简洁、更准确。应合并以下 Entities：阿尔伯特·爱因斯坦、爱因斯坦、A·爱因斯坦、阿尔伯特·爱因斯坦教授',
      ...
      top: '前n条',
      enableAutoGenerateTip: '自动为每个切片生成关键词。',
      llmModel: '大语言模型',
      mineruLanguage: '语言',
      mineruLanguageTip: '传给 MinerU 解析器的语言提示。',
    },
    chunk: {
      chunk: '解析块',
      createChunk: '创建解析块',
    ...
    chat: {
      ...
      emptyResponseTip:
        '如果在知识库中没有检索到用户的问题，它将使用它作为答案。 如果您希望 LLM 在未检索到任何内容时提出自己的意见，请将此留空。仅在思考模式是简单模式时候生效。',
      emptyResponseMessage:
        '当知识库中未检索到任何相关信息时，将触发空回复。由于未选择任何知识库，因此请清除“空回复”。',
      setAnOpener: '设置开场白',
      ...
      topN: 'Top N',
      topNTip:
        '并非所有相似度得分高于“相似度阈值”的块都会被提供给大语言模型。 LLM 只能看到这些“Top N”块。',
      ...
      custom: '自定义',
      freedomTip:
        '“精确”意味着大语言模型会保守并谨慎地回答你的问题。 “即兴发挥”意味着你希望大语言模型能够自由地畅所欲言。 “平衡”是谨慎与自由之间的平衡。',
      temperature: '温度',
      ...
      maxTokensTip:
        '模型的最大上下文大小；无效或不正确的值会导致错误。默认值为 512。',
      ...
      keyword: '关键词分析',
      keywordTip:
        '应用 LLM 分析用户的问题，提取在相关性计算中要强调的关键词。对长查询效果较好，但会延长响应时间。',
      reasoning: '推理',
      reasoningTip:
      ...
      tocEnhanceTip:
        "解析文档时生成了目录信息（见General方法的'启用目录抽取'），让大模型返回和用户问题相关的目录项，从而利用目录项拿到相关 Chunk，对这些 Chunk 在排序中进行加权。这种方法来源于模仿人类查询书本中知识的行为逻辑",
    ...
    setting: {
      ...
      rerankModel: 'Rerank',
      rerankModelTip:
        '默认的 Rerank 模型。如未显示可选模型，请根据 https://ragflow.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      workspace: '工作空间',
      upgrade: '升级',
      addLlmTitle: '添加 LLM',
      ...
      modelProvidersWarn:
        '请先在<b>模型提供商</b>中添加 Embedding 模型和 LLM，然后在“设置默认模型”中设置它们。',
      apiVersion: 'API版本',
    ...
    flow: {
      ...
      exitLoop: '退出循环',
      exitLoopDescription:
        '等同于 "break"。此节点没有配置项。当循环体到达此节点时，循环终止。',
      loopVariables: '循环变量',
      ...
      keywordDescription:
        '该组件用于从用户的问题中提取关键词。Top N 指定需要提取的关键词数量。',
      switchDescription:
