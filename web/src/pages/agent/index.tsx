...
export default function Agent() {
  ...
  /**
   * 智能体 > 名称. The name is the last level, so it carries no link; a canvas with
   * no title yet — a fresh one — reads as 新建智能体 instead of an empty level.
   */
  const breadcrumbTrail = useMemo<BreadcrumbCrumb[]>(
    () => [
      {
        label: agentDetail.title || t('breadcrumb.newAgent'),
      },
    ],
    [agentDetail.title, t],
  );
  ...
  return (
    // The agent editor owns a full-height canvas, so it brings its own shell the
    // way the chat detail does: that is what puts the global header and breadcrumb
    // above it. The breadcrumb this page used to draw inline is now the global
    // one, fed by the trail published below.
    <RootLayoutContainer>
