...
- **Unreproduced team-page reports (needs the reporter's input)**: (1) "clicking the role/department select closes the invite dialog" - three real-browser passes show the dialog surviving, including the disabled-department + tooltip path; the next datum needed is whether the popup appears at all ("never appears" = stacking relative to the modal overlay, "appears then the dialog goes" = the dialog treating a popup portalled to `document.body` as an outside interaction, which this Radix Select does not register a nested-layer branch for); (2) "crash on submit" - no uncaught exception on three submit paths. Each candidate fix is one line in `src/components/ui/{select,modal}.tsx`, which the shared-UI lock reserves for explicit approval.
